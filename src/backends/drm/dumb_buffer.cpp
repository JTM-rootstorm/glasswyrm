...
DumbBuffer::DumbBuffer(DumbBuffer&& other) noexcept { *this = std::move(other); }
...
bool DumbBuffer::create(DumbBufferApi& api, const std::uint32_t width,
  ...
  if (!api.create_dumb(width, height, 32, allocation, error)) return false;
  ...
  if (!api.add_framebuffer2(allocation.handle, width, height, allocation.pitch,
                            kFormatXrgb8888, replacement.framebuffer_id_, error))
    return false;
  ...
  if (!api.map_dumb(allocation.handle, map_offset, error)) return false;
  replacement.mapping_ = api.map_memory(map_offset, replacement.size_, error);
  if (replacement.mapping_ == nullptr) return false;
...
void DumbBuffer::reset() noexcept {
  ...
  if (framebuffer_id_ != 0) api_->remove_framebuffer(framebuffer_id_);
  if (mapping_ != nullptr) api_->unmap_memory(mapping_, size_);
  if (handle_ != 0) api_->destroy_dumb(handle_);
  api_ = nullptr;
  mapping_ = nullptr;
  size_ = 0;
  width_ = 0;
...
bool DumbBufferPair::create(DumbBufferApi& api, const std::uint32_t width,
  ...
  for (auto& buffer : replacement.buffers_)
    if (!DumbBuffer::create(api, width, height, buffer, error)) return false;
  output = std::move(replacement);
  return true;
}
