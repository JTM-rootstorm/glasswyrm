...
std::string serialize(const DrmVrrTimingReport &value) {
  ...
  stream << "{\"record\":\"vrr-timing\",\"commit_id\":"
         ...
         << value.nominal_mode_interval_nanoseconds
         << ",\"effective_enabled\":" << boolean(value.effective_enabled)
         << '}';
  return stream.str();
}
...
std::string serialize(const DrmVrrSummaryReport &value) {
  ...
  stream << "{\"record\":\"vrr-summary\",\"sample_count\":"
         ...
         << value.timestamp_unavailable_count
         << '}';
  return stream.str();
}
...
bool valid(const DrmVrrTimingReport &value) noexcept {
  return value.commit_id != 0 && value.generation != 0 &&
         ...
         value.nominal_mode_interval_nanoseconds != 0;
}

bool valid(const DrmVrrSummaryReport &value) noexcept {
  if (value.sample_count == 0)
    return value.minimum_nanoseconds == 0 &&
           value.maximum_nanoseconds == 0 &&
           value.mean_nanoseconds == 0 &&
           value.median_nanoseconds == 0;
  return value.minimum_nanoseconds != 0 &&
         value.minimum_nanoseconds <= value.maximum_nanoseconds &&
         value.mean_nanoseconds >= value.minimum_nanoseconds &&
         value.mean_nanoseconds <= value.maximum_nanoseconds &&
         value.median_nanoseconds >= value.minimum_nanoseconds &&
         value.median_nanoseconds <= value.maximum_nanoseconds;
}
