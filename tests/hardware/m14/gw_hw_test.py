...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as temporary:
        ...
        invalid_stage_dry_run = run(
            "milestone14-vrr-test", "--config", str(config),
            "--required-base", REQUIRED_BASE,
            "--tested-commit", TESTED_COMMIT,
            "--artifact-dir", str(root / "invalid-stage-dry-run"),
            "--dry-run", "--stage", "stack-cadence", "--yes",
            "--fixture-dir", str(fixture),
        )
        assert invalid_stage_dry_run.returncode == 2
        assert "valid only for a live hardware run" in \
            invalid_stage_dry_run.stderr
