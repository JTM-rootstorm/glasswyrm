...
struct ClientPair {
  ...
  ~ClientPair() { if (peer >= 0) ::close(peer); }
...
void send_setup(ClientPair& pair, const bool big_endian) {
  ...
  const std::array<std::uint8_t, 12> little{'l', 0, 11, 0, 0, 0,
                                             0,   0, 0,  0, 0, 0};
  const std::array<std::uint8_t, 12> big{'B', 0, 0, 11, 0, 0,
                                          0,   0, 0, 0,  0, 0};
  const auto& setup = big_endian ? big : little;
  require(::send(pair.peer, setup.data(), setup.size(), 0) == 12,
          "send setup");
...
int main() {
  ...
  require(::recv(target_client.peer, reply.data(), reply.size(), MSG_WAITALL) ==
              32 && target_client.connection.last_request_sequence() == 1,
  ...
  require(state.resources().create_window(1, 0x00200000U,
  ...
  require(target_event[0] == 19 && target_event[2] == 1 &&
              target_event[3] == 0 && target_event[4] == 1 &&
              parent_event[0] == 19 && parent_event[2] == 0 &&
              parent_event[3] == 2 && parent_event[4] == 0 &&
              parent_event[7] == 1,
  ...
  require(router.route_unmap(spec.xid, state.screen().root_window, clients) == 1,
