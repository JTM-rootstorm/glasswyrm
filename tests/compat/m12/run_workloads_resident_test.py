...
while not (control/'raise-window').exists(): time.sleep(.01)
(control/'raised-ready').write_text('ready\\n')
...
""")
        ...
        def control_handshake() -> None:
            try:
                ...
                (control / "raise-window").write_text("raise\n")
                wait(control / "raised-ready")
