...
The installed C API is version 0.7.0 and the shared library has SOVERSION 0.
...
API 0.7 adds capability-gated eventfd CPU-buffer synchronization.
...
generation from 0.1 through 0.7. Run it after compiling the build tree:

```sh
tests/install/gwipc_staged_consumers_test.sh "$PWD" "$PWD/build-m12"
```
