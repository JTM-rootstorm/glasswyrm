...
bool vrr_output_matches(const Snapshot &snapshot, const std::uint64_t output_id,
  ...
  if (descriptor != snapshot.descriptors.end() &&
    ...
    return true;
  const auto parsed = parse_id(selector);
  return parsed && *parsed == output_id;
}

std::optional<gwipc_vrr_policy_mode>
parse_vrr_policy(const std::string_view value) noexcept {
  if (value == "off")
    return GWIPC_VRR_POLICY_OFF;
  if (value == "fullscreen")
    return GWIPC_VRR_POLICY_FULLSCREEN;
  if (value == "focused")
    return GWIPC_VRR_POLICY_FOCUSED;
  if (value == "app-requested")
    return GWIPC_VRR_POLICY_APP_REQUESTED;
  if (value == "always-eligible")
    return GWIPC_VRR_POLICY_ALWAYS_ELIGIBLE;
  return std::nullopt;
}

const char *vrr_policy_name(const gwipc_vrr_policy_mode value) noexcept {
  switch (value) {
  case GWIPC_VRR_POLICY_OFF:
    return "off";
  case GWIPC_VRR_POLICY_FULLSCREEN:
    return "fullscreen";
  case GWIPC_VRR_POLICY_FOCUSED:
    return "focused";
  case GWIPC_VRR_POLICY_APP_REQUESTED:
    return "app-requested";
  case GWIPC_VRR_POLICY_ALWAYS_ELIGIBLE:
...
const char *
vrr_preference_name(const gwipc_vrr_window_preference value) noexcept {
  switch (value) {
  case GWIPC_VRR_PREFERENCE_DEFAULT:
    return "default";
  case GWIPC_VRR_PREFERENCE_DISABLE:
    return "disable";
  case GWIPC_VRR_PREFERENCE_ALLOW:
    return "allow";
  case GWIPC_VRR_PREFERENCE_PREFER:
    return "prefer";
  }
  return "unknown";
}
...
const char *vrr_decision_name(const gwipc_vrr_decision value) noexcept {
  switch (value) {
  case GWIPC_VRR_DECISION_DISABLED:
    return "disabled";
  case GWIPC_VRR_DECISION_ENABLED:
    return "enabled";
  case GWIPC_VRR_DECISION_UNSUPPORTED:
    return "unsupported";
  case GWIPC_VRR_DECISION_REJECTED:
    return "rejected";
  }
  return "unknown";
}
...
bool apply_vrr_edit(Snapshot &snapshot, const std::string_view selector,
  ...
  for (auto iterator = snapshot.outputs.begin();
    ...
    if (vrr_output_matches(snapshot, iterator->first, selector)) {
      output = iterator;
      break;
    }
  }
