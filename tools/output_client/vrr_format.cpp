...
constexpr std::array kReasons{
    ...
    ReasonName{GWIPC_VRR_REASON_OUTPUT_NOT_VRR_CAPABLE,
               "output-not-vrr-capable"},
    ReasonName{GWIPC_VRR_REASON_ATOMIC_KMS_UNAVAILABLE,
               "atomic-kms-unavailable"},
    ReasonName{GWIPC_VRR_REASON_VRR_PROPERTY_MISSING, "vrr-property-missing"},
    ReasonName{GWIPC_VRR_REASON_VRR_ATOMIC_TEST_FAILED,
    ...
    ReasonName{GWIPC_VRR_REASON_OUTPUT_CONFIGURATION_BUSY,
    ...
    ReasonName{GWIPC_VRR_REASON_WINDOW_NOT_BORDERLESS_FULLSCREEN,
               "window-not-borderless-fullscreen"},
    ReasonName{GWIPC_VRR_REASON_WINDOW_SPANS_OUTPUTS, "window-spans-outputs"},
    ReasonName{GWIPC_VRR_REASON_WINDOW_PREFERENCE_DISABLED,
               "window-preference-disabled"},
    ReasonName{GWIPC_VRR_REASON_WINDOW_DID_NOT_REQUEST,
...
void text_reasons(std::ostream &output, const std::uint64_t reasons) {
  output << '[';
  bool comma = false;
  for (const auto name : vrr_reason_names(reasons)) {
    if (comma)
...
void print_vrr(const Snapshot &snapshot,
  ...
  for (const auto &[output_id, capability] : snapshot.vrr_capabilities) {
    ...
    if (selector && !vrr_output_matches(snapshot, output_id, *selector))
      continue;
    ...
    if (json) {
      if (!first)
        output << ',';
      output << "{\"id\":\"" << id(output_id)
             << "\",\"name\":" << json_string(name) << ",\"policy\":\""
             ...
             << ",\"simulated\":" << (capability.simulated ? "true" : "false")
             << ",\"range_millihertz\":";
      if (capability.range_available)
        output << '[' << capability.minimum_refresh_millihertz << ','
               << capability.maximum_refresh_millihertz << ']';
      else
        output << "null";
      if (state != snapshot.vrr_outputs.end())
        output << ",\"decision\":\""
               << vrr_decision_name(state->second.decision)
               << "\",\"desired_enabled\":"
               ...
               << ",\"interval_ns\":"
               << state->second.last_interval_nanoseconds;
      ...
      output << id(output_id) << ' ' << name << " policy="
             << vrr_policy_name(policy == snapshot.vrr_policies.end()
                                    ? GWIPC_VRR_POLICY_OFF
                                    : policy->second)
             << " property_present=" << capability.connector_property_present
             << " hardware=" << capability.hardware_capable
             << " controllable=" << capability.kms_controllable
             << " simulated=" << capability.simulated << " range=";
      if (capability.range_available)
        output << capability.minimum_refresh_millihertz << '-'
               << capability.maximum_refresh_millihertz;
      else
  ...
  if (json) {
    ...
    for (const auto &[window_id, window] : snapshot.vrr_windows) {
      if (selector) {
        if (!vrr_output_matches(snapshot, window.output_id, *selector))
          continue;
      }
      if (!first)
        output << ',';
      output << "{\"window\":" << window_id << ",\"surface\":\""
             ...
             << ",\"selected\":" << (window.policy_selected ? "true" : "false")
    ...
    for (const auto &[window_id, window] : snapshot.vrr_windows) {
      ...
      output << "window=" << window_id << " surface=" << id(window.surface_id)
             << " output=" << id(window.output_id)
             ...
             << " policy_generation=" << window.policy_generation
             << " reasons=";
      text_reasons(output, window.reason_flags);
      output << '\n';
    }
  }
}
