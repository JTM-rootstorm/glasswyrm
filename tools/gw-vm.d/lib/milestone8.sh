...
"$runtime_build_dir/tests/x11_milestone8_probe" --display :99 --input-socket "$input_socket" --byte-order little --scenario propagation >>"$events_log" 2>&1; exposure_events=passed
"$runtime_build_dir/tests/x11_milestone8_probe" --display :99 --input-socket "$input_socket" --byte-order little --scenario errors >>"$malformed_log" 2>&1; malformed_x11=passed
...
collect_milestone8_artifacts() {
  ...
  [[ ! -f "$ARTIFACTS_PATH_ABS/milestone8-input-rendering.tar" ]] || tar -tf "$ARTIFACTS_PATH_ABS/milestone8-input-rendering.tar" >/dev/null || failed=1
  return "$failed"
}
