...
systemd-run --unit=gwcomp-m10 "${service_hardening[@]}" --setenv=GW_ALLOW_HARDWARE_TESTS=1 --property=RuntimeDirectory=glasswyrm-m10-gwcomp --property=PrivateDevices=no --property=DevicePolicy=closed --property="DeviceAllow=$drm_device rw" --property="DeviceAllow=$target_vt rw" --property=StandardInput=tty-force --property="TTYPath=$target_vt" --property=TTYReset=yes --property=TTYVHangup=yes --property=TTYVTDisallocate=no --no-block -- "$runtime/src/gwcomp" --backend drm --ipc-socket "$comp_socket" --drm-device "$drm_device" --tty "$target_vt" --connector "$connector" --mode 1024x768 --drm-api auto --mirror-dump-dir "$dumps" --scene-manifest "$scenes/scene.jsonl" --drm-report "$artifact_dir/milestone10-drm-report.jsonl"
