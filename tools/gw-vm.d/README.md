...
state. They require `--yes` unless the relevant safety setting explicitly
allows the operation. Shutdown requests a graceful guest shutdown; the harness
does not destroy a running domain by default.
